...
async function main() {
  ...
  const agree = { mastery: 0, days: 0, historyRows: 0, queue: 0, phoneXp: 0 };
  const diffs = [];
  let withEvents = 0;
  for (const u of users) {
    ...
    for (const [ch, m] of Object.entries(u.chapterMastery || {})) diagnosticScores[ch] = m.diagnosticScore || 0;
    const d = deriveAccount({ events, diagnosticScores });
    ...
    // phone XP is not stored on its own; compare the day count only
    agree.phoneXp++;

    const score = masteryOff.length + (daysOff ? 0 : 1) + rowsOff.length + queueOff.length;
    if (score) diffs.push({ email: u.email.replace(/^(..).*(@.*)$/, '$1…$2'), events: events.length, stored: stored.length, derived: Object.keys(d.history).length,
      ...
      days: `${storedDays.size}/${d.studyDays.length}`, rowsOff: rowsOff.length, queueOff: queueOff.length, score });
  }
  diffs.sort((a, b) => b.score - a.score);
  console.log(`accounts ${users.length}, with events ${withEvents}`);
  for (const k of ['mastery', 'days', 'historyRows', 'queue']) console.log(`  agree on ${k.padEnd(12)} ${agree[k]} of ${users.length}`);
